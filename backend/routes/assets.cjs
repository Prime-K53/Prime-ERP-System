...
router.get('/', (req, res) => {
  try {
    const assets = db.prepare('SELECT * FROM assets WHERE company_id = ? ORDER BY created_at DESC').all(req.companyId);
    res.json(assets);
  } catch (err) {
...
router.post('/', (req, res) => {
  try {
    ...
    db.prepare(`INSERT INTO assets (id, name, asset_type, serial_number, model, manufacturer, purchase_date, purchase_cost, current_value, useful_life_years, status, location, assigned_to, notes, warranty_expiry, company_id, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
