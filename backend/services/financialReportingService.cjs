...
class FinancialReportingService extends BaseService {
   ...
   */
  async getProfitAndLoss(companyId, startDate, endDate, currency = 'USD') {
    try {
      // Revenue
        ...
        this.db.get(
          `SELECT COALESCE(SUM(total_amount), 0) as total_revenue
           ...
           WHERE company_id = ?
             ...
             AND date(created_at) <= ?
             AND currency = ?`,
          [companyId, startDate, endDate, currency],
          (err, row) => {
            if (err) reject(err);
          ...
          `SELECT COALESCE(SUM(amount), 0) as total_cogs
           ...
           WHERE company_id = ?
             ...
             AND account_id IN (
               ...
               WHERE company_id = ? AND code LIKE '5%'
             )
             AND date(entry_date) >= ?
             AND date(entry_date) <= ?
             AND currency = ?`,
          [companyId, companyId, startDate, endDate, currency],
          (err, row) => {
            if (err) reject(err);
        ...
        this.db.get(
          `SELECT COALESCE(SUM(amount), 0) as total_expenses
           ...
           WHERE company_id = ?
             ...
             AND account_id IN (
               SELECT id FROM chart_of_accounts
               WHERE company_id = ? AND code LIKE '6%'
             )
             AND date(entry_date) >= ?
             AND date(entry_date) <= ?
             AND currency = ?`,
   ...
   */
  async getBalanceSheet(companyId, asOfDate, currency = 'USD') {
    try {
      // Assets
        ...
        this.db.get(
          `SELECT COALESCE(SUM(CASE WHEN entry_type = 'debit' THEN amount ELSE -amount END), 0) as net_cash
           ...
           WHERE company_id = ?
             AND account_id IN (
               SELECT id FROM chart_of_accounts
               WHERE company_id = ? AND code IN ('1200', '4000', '5000', '6000')
             )
             AND date(entry_date) >= ?
             AND date(entry_date) <= ?`,
        ...
        this.db.get(
          `SELECT COALESCE(SUM(CASE WHEN entry_type = 'debit' THEN -amount ELSE amount END), 0) as net_cash
           ...
           WHERE company_id = ?
             AND account_id IN (
               SELECT id FROM chart_of_accounts
               WHERE company_id = ? AND code LIKE '1%' AND code != '1200'
             )
             AND date(entry_date) >= ?
             AND date(entry_date) <= ?`,
          [companyId, companyId, startDate, endDate],
   ...
   */
  async getARAging(companyId, asOfDate) {
    try {
   ...
   */
  async getAPAging(companyId, asOfDate) {
    try {
   ...
   */
  async getBudgetVsActual(companyId, fiscalYear, period) {
    try {
   ...
   */
  async getVATReport(companyId, period) {
    try {
