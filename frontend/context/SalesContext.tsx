...
export const SalesProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const convertQuotationToInvoice = async (q: Quotation): Promise<string> => {
        // Extract the number part from quotation ID to preserve it in invoice
        // e.g., QTN-P7260001 -> INV-P7260001
        const quotationIdMatch = q.id.match(/([A-Z]+)-(.+)/);
        let invId: string;
        
        if (quotationIdMatch) {
            // Get invoice prefix from config, default to 'INV'
            const invoiceRule = companyConfig?.transactionSettings?.numbering?.invoice;
            const invoicePrefix = invoiceRule?.prefix || 'INV';
            // Use the same number part as the quotation
            invId = `${invoicePrefix}-${quotationIdMatch[2]}`;
            
            // Check if this invoice ID already exists (e.g., if INV-xxx02 was created directly)
            const existingInvoice = finance.invoices.find(inv => inv.id === invId);
            if (existingInvoice) {
                throw new Error(`Cannot convert quotation: Invoice ${invId} already exists. This may occur if an invoice with this number was created directly.`);
            }
        } else {
            // Fallback to generating new ID if format doesn't match
            invId = generateNextId('invoice', finance.invoices, companyConfig);
        }
        ...
        try {
            ...
            // Update local quotation status to Converted
