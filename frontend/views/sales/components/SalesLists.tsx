...
export const InvoiceList: React.FC<ListProps<Invoice>> = (props) => {
    ...
    const isRecordSelected = (id: string) => props.selectedIds?.includes(id);
                        ...
                        <table className="w-full text-left text-[13px]">
                            ...
                            <tbody className="divide-y divide-slate-100/50">
                                {(currentItems || []).map((inv: any) => {
                                    ...
                                    const isPartial = inv.status === 'Partial' || (inv.paidAmount || 0) > 0;
                                    const isChecked = isRecordSelected(inv.id);
...
export const QuotationList: React.FC<ListProps<Quotation>> = (props) => {
    ...
    const renderMenu = (quote: Quotation) => {
        ...
        const x = menuPos ? Math.min(menuPos.x, window.innerWidth - menuWidth) : 0;
        const y = menuPos ? Math.min(menuPos.y, window.innerHeight - menuHeight) : 0;
...
export const RecurringList: React.FC<ListProps<RecurringInvoice>> = (props) => {
    ...
    const renderMenu = (sub: RecurringInvoice) => {
        ...
        return (
            <div
                ref={menuRef}
                className="fixed w-64 bg-white/90 backdrop-blur-md rounded-2xl shadow-2xl border border-slate-200 z-[200] animate-in fade-in zoom-in-95 duration-100 flex flex-col py-1 text-left overflow-y-auto custom-scrollbar"
                style={{ 
