        ...
        if (action === 'convert_to_order' && activeView === 'Quotations') {
            if (window.confirm("Convert this quotation to an active order? This will mark the quotation as 'Converted'.")) {
                try {
                    ...
                    if (orderId) {
                        notify(`Quote ${item.id} successfully converted to Order ${orderId}`, "success");
                        setActiveTab('Orders');
                    }
                } catch (err: any) {
                    notify(`Order Conversion Failed: ${err.message}`, "error");
                }
            }
            return;
        }

        if (action === 'record_payment' && activeView !== 'Orders' && activeView !== 'Invoices') {
            return;
        }

        if (action === 'convert_inv' && activeView === 'Quotations') {
            try {
                const invId = await convertQuotationToInvoice(item);
                if (invId) {
                    notify(`Quote ${item.id} successfully converted to Invoice ${invId}`, "success");
                    setActiveTab('Invoices');
                }
            } catch (err: any) {
                notify(`Invoice Conversion Failed: ${err.message}`, "error");
            }
            return;
        }

        if (action === 'convert_to_invoice' && activeView !== 'Orders' && activeView !== 'Quotations') {
            return;
        }
