  ...
  onSaveSettings
    ...
    const loadSettings = async () => {
      ...
      try {
        ...
        if (settingsData) {
          if (!settingsData.paper_item_id) {
            ...
            if (defaultPaper) {
              ...
              settingsData.paper_unit_cost = Number((defaultPaper as Item & Record<string, unknown>).cost_per_unit ?? defaultPaper.cost ?? 0);
            }
          }
          ...
          if (!settingsData.toner_item_id) {
            ...
            if (defaultToner) {
              ...
              settingsData.toner_unit_cost = Number((defaultToner as Item & Record<string, unknown>).cost_per_unit ?? defaultToner.cost ?? 0);
            }
          }
        }
        ...
        logger.error('Error loading examination pricing settings:', loadError);
    ...
    const handleSave = async () => {
      ...
      if (isExternal && onSaveSettings) {
        ...
        try {
          ...
          logger.error('Error saving via parent:', saveError);
      ...
      try {
        ...
        if (batch?.id) {
          try {
            ...
            logger.error('[ExaminationPricingSettings] Failed to sync pricing to classes:', syncError);
        ...
        logger.error('Error saving examination pricing settings:', saveError);
        setError('Failed to save pricing settings.');
      } finally {
        setSaving(false);
      }
    ...
    const handlePaperSelection = (value: string) => {
      ...
      setSettings({
        ...
        paper_unit_cost: selected ? Number((selected as Item & Record<string, unknown>).cost_per_unit ?? selected.cost ?? selected.cost_price ?? 0) : 0
      });
    };
    ...
    const handleTonerSelection = (value: string) => {
      ...
      setSettings({
        ...
        toner_unit_cost: selected ? Number((selected as Item & Record<string, unknown>).cost_per_unit ?? selected.cost ?? selected.cost_price ?? 0) : 0
      });
    };
