            ...
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex bg-slate-100 p-1 rounded-xl">
                    {['Issued', 'Received', 'Print'].map(tab => (
                        <button 
                            key={tab} 
                            onClick={() => setActiveTab(tab as 'Issued' | 'Received' | 'Print')}
                                    ...
                                    {[
                                        ...
                                        <div key={field.key}>
                                            ...
                                            <div className="grid grid-cols-2 gap-2 mt-1">
                                                <input type="number" className="p-1 border rounded text-xs" value={(printConfig as Record<string, { x: number; y: number }>)[field.key].x} onChange={e => setPrintConfig({...printConfig, [field.key]: { ...(printConfig as Record<string, { x: number; y: number }>)[field.key], x: Number(e.target.value) }})}/>
                                                <input type="number" className="p-1 border rounded text-xs" value={(printConfig as Record<string, { x: number; y: number }>)[field.key].y} onChange={e => setPrintConfig({...printConfig, [field.key]: { ...(printConfig as Record<string, { x: number; y: number }>)[field.key], y: Number(e.target.value) }})}/>
                                            </div>
                                        </div>
                                    ))}
                                </div>
