...
const BarcodePrinter: React.FC = () => {
    ...
    }, [inventory, isLoading, fetchInventory]);
    ...
    useEffect(() => {
        ...
        if (itemId) {
            ...
            if (found) {
                setPrintQueue(prev => {
                    if (prev.some(p => p.item.id === found.id)) return prev;
    ...
    const filteredItems = term
        ? items.filter(i =>
            ...
            ((i && i.sku) || '').toLowerCase().includes(term)
          ).slice(0, 10)
        : items.slice(0, 10);
                    ...
                    <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                        ...
                        <div className="space-y-2 max-h-[300px] overflow-y-auto custom-scrollbar">
                            {isLoading ? (
                                ...
                                <div className="text-center py-8 text-xs text-slate-400">No items found. Add inventory items first.</div>
                            ) : filteredItems.length === 0 ? (
                                <div className="text-center py-8 text-xs text-slate-400">No items matching "{searchTerm}"</div>
                            ) : (
