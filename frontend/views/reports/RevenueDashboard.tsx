...
const RevenueDashboard: React.FC = () => {
    ...
    const stats = useMemo(() => {
        ...
        // Deduplicate sales and invoices by ID to prevent double counting
        const uniqueRevenueMap = new Map<string, any>();
        filteredInvoices.forEach((i: any) => uniqueRevenueMap.set(i.id, i));
        filteredSales.forEach((s: any) => uniqueRevenueMap.set(s.id, s));
        const uniqueRevenueTransactions = Array.from(uniqueRevenueMap.values());

        // Total Revenue (Sales + Invoices uniquely)
        const totalRevenue = uniqueRevenueTransactions.reduce((sum: number, t: any) => sum + (t.totalAmount || t.total || 0), 0);
        ...
        // Total Expenses (Restricted to Procurement ONLY)
        const totalExpenses = filteredPurchases.reduce((sum: number, p: any) => sum + (p.totalAmount || p.total || p.amount || 0), 0);
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const dayPurchases = purchases
                .filter((p: any) => p.date && p.date.startsWith(dateStr))
                .reduce((sum: number, p: any) => sum + (p.totalAmount || p.total || p.amount || 0), 0);
