  ...
  const generate = useCallback(async (id: number) => {
    ...
    try {
      ...
      console.log('[Preview Payload]', JSON.stringify({
        documentType: type,
        itemsLength: Array.isArray(previewData.items) ? (previewData.items as unknown[]).length : 'missing',
        hasCustomer: !!(previewData.clientName || previewData.customerName),
        totalAmount: previewData.totalAmount,
        documentNumber: previewData.number || previewData.invoiceNumber,
