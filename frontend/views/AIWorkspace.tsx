...
import {
  Sparkles, Send, User, TrendingUp, DollarSign, Users, FileText,
    ...
    try {
      ...
      if (/summary|overview|finances/.test(lower)) {
          ...
          const scored = customers
            .map((c) => {
              ...
              return calculateCustomerRiskScore(
                ...
                [],
                (sales || []).filter((s) => (s.customerId || '') === id)
              );
            })
        ...
        if (unpaidInvoices.length === 0) {
          ...
          const totalUnpaid = unpaidInvoices.reduce((s, inv) => s + (inv.totalAmount || 0), 0);
          response =
    ...
    return text.split('\n').map((line, i) => (
      <span key={i}>
        {line.startsWith('• ') || line.startsWith('  ') ? (
          <span style={{ display: 'block', paddingLeft: line.startsWith('  ') ? 20 : 0 }}>
            {line.startsWith('  ') ? line : `• ${line.slice(2)}`}
          </span>
        ) : line.startsWith('**') && line.endsWith('**') ? (
          <span style={{ fontWeight: 700, fontSize: 15, display: 'block', marginTop: 8, marginBottom: 4 }}>
            {line.slice(2, -2)}
          </span>
        ) : line.startsWith('*') && line.endsWith('*') ? (
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        ...
        <div>
          <div style={styles.headerText}>AI Finance Workspace</div>
          <div style={styles.headerSub}>Intelligent insights powered by machine learning</div>
        </div>
      </div>
      ...
      <div style={styles.statsRow}>
        {[
          ...
          { icon: <Users size={18} />, color: '#10b981', bg: '#d1fae5', value: activeCustomersCount.toString(), label: 'Active Customers' },
          { icon: <FileText size={18} />, color: '#f59e0b', bg: '#fef3c7', value: unpaidInvoices.length.toString(), label: 'Pending Invoices' },
          { icon: <Sparkles size={18} />, color: '#8b5cf6', bg: '#ede9fe', value: aiInsightsCount.toString(), label: 'AI Insights Today' },
        ].map((stat, i) => (
