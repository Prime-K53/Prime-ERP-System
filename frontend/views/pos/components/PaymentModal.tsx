...
export const PaymentModal: React.FC<PaymentModalProps> = ({
    ...
    // Ref to store addPaymentMethod for keyboard shortcuts (avoids re-creating event listener)
    const addPaymentMethodRef = useRef<(accountId: string) => void>(() => {});
    ...
    const addPaymentMethod = useCallback((accountId: string) => {
        // If input is empty or invalid, use the remaining due amount
        let amountInput = parseFloat(currentPaymentAmount);
        if (isNaN(amountInput) || amountInput <= 0) {
            // Use remaining due if available, otherwise use total
            amountInput = remainingDue > 0.01 ? remainingDue : total;
            if (amountInput <= 0) {
        ...
        // Don't clear the input - keep remaining amount for easier split payments
        setCurrentPaymentAmount(newRemaining > 0.01 ? newRemaining.toFixed(2) : '');
    }, [currentPaymentAmount, splitPayments, total, remainingDue, notify]);

    // Update ref to always point to latest addPaymentMethod
    useEffect(() => {
        addPaymentMethodRef.current = addPaymentMethod;
    });

    // Keyboard Shortcuts Logic - uses ref to avoid re-creating event listener
    useEffect(() => {
        const handleGlobalKeys = (e: KeyboardEvent) => {
            ...
            if (e.altKey) {
                if (e.key === '1') addPaymentMethodRef.current('1000');
                if (e.key === '2') addPaymentMethodRef.current('1050');
                if (e.key === '3') addPaymentMethodRef.current('1060');
            }
        };
        window.addEventListener('keydown', handleGlobalKeys);
        return () => window.removeEventListener('keydown', handleGlobalKeys);
    }, [canCompleteSale, handleComplete, onCancel]);
