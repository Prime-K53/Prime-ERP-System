...
const MigrationHealth: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (migrationDoc) {
        setMigrationState((migrationDoc as { value: unknown }).value);
      }
    } catch (err) {
      logger.error('MigrationHealth: error loading data', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadTableCounts = async (): Promise<Record<string, number>> => {
    try {
      ...
      for (const tableName of ALL_TABLES) {
        const table = (db as Record<string, { count(): Promise<number> } | undefined>)[tableName];
        if (table) {
