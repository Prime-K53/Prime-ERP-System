...
const PrintingServiceModal: React.FC<Props> = ({ item, onSave, onClose, allItems = [] }) => {
  ...
  const calculateCosts = () => {
    ...
    const finishingCostVal = finishingOptions
      .filter(o => o.enabled)
      .reduce((sum, o) => sum + (o.price * copies), 0);
  ...
  const suggestedSellingPrice = useMemo(() => {
    ...
    return parseFloat((costPrice * (1 + DEFAULT_MARKUP_PCT / 100)).toFixed(2));
  }, [costPrice]);
  ...
  const renderBasicInfoTab = () => (
    <div className="space-y-4">
      ...
      <div className={premiumCard}>
        {renderCardHeader(<Sparkles size={15} className="text-white" />, 'AI Description', 'Auto-generated', 'from-cyan-500 to-teal-600')}
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            <button type="button" onClick={handleAiDescribe} className="flex items-center gap-1.5 px-4 py-2 bg-gradient-to-r from-cyan-500 to-teal-600 text-white rounded-lg text-xs font-semibold hover:from-cyan-600 hover:to-teal-700 transition-all shadow-lg shadow-cyan-200">
              <Sparkles size={14} /> Generate Description
            </button>
            {showAiDescription && <span className="text-[10px] text-teal-600 bg-teal-50 px-2 py-1 rounded-full">AI-generated</span>}
          </div>
          {showAiDescription && (
            <textarea value={generatedDescription} onChange={e => setGeneratedDescription(e.target.value)}
              className={`${premiumInput} resize-none w-full`} rows={3} placeholder="AI-generated description will appear here..." />
          )}
        </div>
      </div>
    </div>
