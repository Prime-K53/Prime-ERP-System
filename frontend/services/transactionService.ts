...
const resolveBankAccountForPayment = (
    ...
    // 2. Map via GL Config (Reliable link)
    const gl = getGLConfig();
    const method = (payment.paymentMethod || '').toLowerCase();
    const accountId = payment.accountId || '';

    // Helper for fuzzy matching (fallback)
    ...
    // Route based on GL assignment or fallback to keyword match
    if (accountId === gl.cashDrawerAccount || method.includes('cash')) {
        const found = bankAccounts.find(acc => acc.accountNumber === gl.cashDrawerAccount || acc.id === gl.cashDrawerAccount);
        return found || bankAccounts.find(acc => matches(acc, ['cash']));
    }

    if (accountId === gl.mobileMoneyAccount || method.includes('mobile') || method.includes('momo')) {
        const found = bankAccounts.find(acc => acc.accountNumber === gl.mobileMoneyAccount || acc.id === gl.mobileMoneyAccount);
        return found || bankAccounts.find(acc => matches(acc, ['mobile', 'momo']));
    }

    if (accountId === gl.bankAccount || method.includes('bank') || method.includes('card')) {
        const found = bankAccounts.find(acc => acc.accountNumber === gl.bankAccount || acc.id === gl.bankAccount);
        return found || bankAccounts.find(acc => matches(acc, ['bank']));
    }
