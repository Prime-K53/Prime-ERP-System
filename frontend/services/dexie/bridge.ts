...
export const dexieBridge = {
  ...
  async put(storeName: BackedLegacyStoreName, value: unknown) {
    ...
    const repos = await getRepositories();
    switch (storeName) {
      case 'inventory': await repos.products.upsert(normalizeLegacyProduct(value, 'user-action')); return String((value as { id?: string })?.id || '');
      case 'customers': await repos.customers.upsert(normalizeLegacyCustomer(value, 'user-action')); return String((value as { id?: string })?.id || '');
      default: return String((value as { id?: string })?.id || '');
    }
  },
