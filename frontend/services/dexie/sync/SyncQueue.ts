...
export class SyncQueue {
  async enqueue(item: Omit<SyncQueueItem, 'id' | 'createdAt' | 'updatedAt' | 'status' | 'retries'>): Promise<SyncQueueItem> {
    ...
    const existing = await table
      .filter((e) => e.extra?.dedupeKey === item.dedupeKey)
      .first();
  ...
  async updateStatus(id: string, status: SyncOperationStatus, error?: unknown): Promise<void> {
    ...
    if (status === 'failed') {
      ...
      if (existing) {
        patch.retries = existing.retries + 1;
        patch.nextRetryAt = new Date(Date.now() + this.buildBackoff(existing.retries + 1)).toISOString();
      }
    }
