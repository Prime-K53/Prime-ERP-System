  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      [...notifications, ...rows].forEach((entry) => {
        ...
        const ent = entry as Notification | NotificationEntity;
        if (!current || new Date(current.timestamp).getTime() <= new Date(
          ((ent as Notification).timestamp instanceof Date ? (ent as Notification).timestamp.toISOString() : '') ||
          (ent as NotificationEntity).createdAt
        ).getTime()) {
          merged.set(entry.id, fromRepositoryEntity(ent as NotificationEntity));
        }
      });
