  ...
  async getSyncQueue(statuses?: SyncQueueStatus[]): Promise<SyncQueueItem[]> {
    if (shouldUseDexie('syncOperations') && dexieQueueCoordinator.supportsDexie()) {
      try {
        ...
        const mergedRows = normalizeQueueRows(mergeByIdentifier<SyncQueueItem>((rows as SyncQueueItem[]) || [] as SyncQueueItem[], legacyRows), statuses);
        if (mergedRows.length > 0) {
          return mergedRows;
        }
      } catch {
    ...
    if (shouldUseDexie('syncOperations') && dexieQueueCoordinator.supportsDexie()) {
      try {
        const queued = await Promise.race([
          dexieQueueCoordinator.enqueue(next),
          new Promise<null>((resolve) => setTimeout(() => resolve(null), 10000))
        ]);
        if (queued) {
    ...
    if (shouldUseDexie('syncOperations') && dexieQueueCoordinator.supportsDexie()) {
      try {
        const saved = await Promise.race([
          dexieQueueCoordinator.save(next),
          new Promise<null>((resolve) => setTimeout(() => resolve(null), 10000))
        ]);
