...
export const referralReversalService = {
  ...
  async approveReversal(reversalId: string, approvedBy: string): Promise<ReversalRequest> {
    ...
    await dbService.executeAtomicOperation(
      ['walletTransactions', 'customers', 'ledger', 'referralRewards', 'referralReversals', 'idempotencyKeys'],
      async (tx) => {
        const walletTx = {
          id: walletTxId,
        ...
        if (referrerCustomerId) {
          ...
          if (referrer) {
            ...
            await tx.objectStore('customers').put(referrer)
          }
        }
        ...
        await tx.objectStore('ledger').put(ledgerEntry)
        ...
        await tx.objectStore('referralRewards').put(reward)
        ...
        reversal.updatedAt = new Date().toISOString()
        await tx.objectStore('referralReversals').put(reversal)
      }
    )
  ...
  async getAllReversals(): Promise<ReversalRequest[]> {
    return dbService.getAll<ReversalRequest>('referralReversals')
  },
}
