...
/* ───────── Migrated Inline Features ───────── */

export async function generateArchitectDoc(prompt: string) {
  return aiService.generateArchitectDoc(prompt);
}

export async function generateBusinessMessage(context: string, requirement: string) {
  return aiService.generateBusinessMessage(context, requirement);
}

export async function askFullAssistant(context: string, question: string) {
  return aiService.askFullAssistant(context, question);
}
...
export async function analyzePredictiveMaintenance(
  ...
  return aiService.analyzePredictiveMaintenance(machineName, temperature, vibration, efficiency, uptime);
}

export async function analyzeInkDensity() {
  return aiService.analyzeInkDensity();
}

export async function generateSupplyChainStrategy(
  itemName: string, stock: number, adu: number, daysUntilStockout: number, marginPercent: number,
) {
  return aiService.generateSupplyChainStrategy(itemName, stock, adu, daysUntilStockout, marginPercent);
}
...
export async function generatePricingStrategy(
  ...
  return aiService.generatePricingStrategy(itemName, currentPrice, linkedBom, bomDetails, actualBomCost, laborCost, wastePct, lastCost);
}

export async function generateBillingEmail(
  ...
  return aiService.generateBillingEmail(type, id, customerName, total, currency, dueDate, status, companyName);
}

export async function generateCollectionsStrategy(
  customerName: string, id: string, totalAmount: number,
  currency: string, dueDate: string, status: string,
) {
  return aiService.generateCollectionsStrategy(customerName, id, totalAmount, currency, dueDate, status);
}

export async function runPreflightAudit(jobTitle: string, jobDescription: string, attachments: string) {
  return aiService.runPreflightAudit(jobTitle, jobDescription, attachments);
}

export async function askFloatingAssistant(context: string, question: string) {
  return aiService.askFloatingAssistant(context, question);
}

export async function testConnection(
  provider: any, apiKey: string, model: string, baseUrl?: string,
) {
  return aiService.testConnection(provider, apiKey, model, baseUrl);
}
