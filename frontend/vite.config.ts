...
export default defineConfig(({ mode }) => {
    ...
    return {
      ...
      optimizeDeps: { include: ['recharts','lucide-react','react-router-dom','idb','date-fns','@react-pdf/renderer','zustand','dexie'], exclude: ['@supabase/supabase-js','yoga-layout'] },
      ...
      build: { outDir: 'dist', emptyOutDir: true, manifest: 'asset-manifest.json', sourcemap: false, rollupOptions: { shimMissingExports: true, output: { manualChunks: { react: ['react', 'react-dom'], vendor: ['date-fns', 'zustand', 'idb', 'dexie'] } } } }
    };
});
