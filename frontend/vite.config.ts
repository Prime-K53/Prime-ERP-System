...
export default defineConfig(({ mode }) => {
    ...
    return {
      ...
      build: { outDir: 'dist', emptyOutDir: true, manifest: 'asset-manifest.json', sourcemap: false, rollupOptions: { shimMissingExports: true, output: { manualChunks(id) { if (id.includes('node_modules/lucide-react')) return 'icons'; if (id.includes('node_modules/recharts')) return 'charts'; if (id.includes('node_modules/@supabase')) return 'supabase'; if (id.includes('node_modules/react-pdf') || id.includes('node_modules/@react-pdf')) return 'pdf'; if (id.includes('node_modules/react') || id.includes('node_modules/react-dom') || id.includes('node_modules/scheduler')) return 'react'; if (id.includes('node_modules/dexie')) return 'dexie'; if (id.includes('node_modules/zustand')) return 'zustand'; if (id.includes('node_modules/idb')) return 'idb'; if (id.includes('node_modules/date-fns')) return 'date-fns'; if (id.includes('node_modules')) return 'vendor'; } } } }
    };
});
